...
void cylinder_half_param(Block2d& block)
{
    ...
    block.grid_path=add_mesh_directory_modify_for_linux()+"/../mesh_file/cylinder-half.neu";  //物理文件（.neu）的路径
...
//初场条件
void cylinder_half_init_param(Block2d& block,std::vector<Fluid2d>& fluids)
{
...
void cylinder_half()
{
    ...
    //流场初始量输入
    cylinder_half_init_param(block,fluids);
    //流场初始化
