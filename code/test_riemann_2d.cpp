...
void riemann_2d_param(Block2d& block)
{
    ...
    block.grid_path=add_mesh_directory_modify_for_linux()+"/../mesh_file/100x100.neu";  //物理文件（.neu）的路径
...
//初场条件
void riemann_2d_init_param(Block2d& block,std::vector<Fluid2d>& fluids)
{

    for (auto& fluid : fluids) {
        

        if(fluid.x<0.7 && fluid.y<0.7)
        {
            fluid.rho = 0.138;
            fluid.vx=1.206;
            fluid.vy=1.206;
            fluid.pressure=0.029;   
        }
...
void riemann_2d()
{
    ...
    //流场初始量设置
    riemann_2d_init_param(block,fluids);
    //流场初始化
