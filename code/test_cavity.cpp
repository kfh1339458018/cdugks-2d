...
void cavity_param(Block2d& block)
{
    ...
    block.grid_path=add_mesh_directory_modify_for_linux()+"/../mesh_file/hello.neu";  //物理文件（.neu）的路径
    block.velspace_type=VELSPACE_TYPE::Unstruct;  
    block.vel_path=add_mesh_directory_modify_for_linux()+"/../mesh_file/unstruct_1192.neu";  //非结构速度文件（.neu）的路径   unstruct_1192

    block.endtime_type=ENDTIME_TYPE::Converge;
    block.EndTime=1000000;  //迭代步数
    ...
    block.temp_init=1.2;  //参考温度
    //block.Ma=0.1;  //参考马赫数
    //block.Kn=2.6765197e-4/1.624;  //初始努森数
    block.Ma=0.1624;
    block.Kn=0.075;
    //block.Re;  //初始雷诺数
    block.omega=0.5;  //omega,计算粘温变化时用，硬球分子模型omega=0.5
    block.K_limit=0.5;  //限制器中的可调参数K_limit
    block.error_limit=1e-6;
    
}

//初场条件
void cavity_init_param(Block2d& block,std::vector<Fluid2d>& fluids)
{
    ...
    for (auto& fluid : fluids) {
        ...
        fluid.pressure = fluid.rho*block.R*fluid.temp; // 计算压强
        
        //计算声速
        double sound_speed=sqrt(block.gama*block.R*block.temp_init);
        fluid.vx = 0.0*sound_speed;
        fluid.vy = 0.0*sound_speed;
    } 
}
...
void cavity()
{
    ...
    //流场初始量设置
    cavity_init_param(block,fluids);
    //流场初始化
